    ...
    setChartDetails(prev => {
      return {
        ...
        toolTip: {
          ...
          msg: `Month: ${chartDetails.horiArrow.months[e.target.name()]} \n Value:${chartDetails.dataSet[e.target.name()]}`
        }
      }
    })
  }
      ...
      <Layer>
        <Group draggable height={chartDetails.graphHeight} width={chartDetails.graphWidth} x={chartDetails.graphCoordX} y={chartDetails.graphCoordY} onDragStart={onMouseOut} ref={groupRef}>
          ...
          {chartDetails.dataSet.map((rect, i) => <Fragment key={i}>
            ...
            <Text key={-i - chartDetails.dataSet.length} text={months[i]} fontSize={chartDetails.graphWidth > 300 ? 15 : 10} x={30 + chartDetails.padding + i * chartDetails.xScale} y={chartDetails.graphHeight - chartDetails.padding / 1.1} visible={chartDetails.dataSet.length < 5 || chartDetails.graphWidth > 200} />
            {console.log(chartDetails.horiArrow.xAxis)}
          </Fragment>)}
        </Group>
