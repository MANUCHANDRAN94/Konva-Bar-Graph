    ...
    <Stage width={window.innerWidth} height={window.innerHeight}  >
      <Layer>
        <Group draggable height={chartDetails.graphHeight} width={chartDetails.graphWidth} x={chartDetails.graphCoordX} y={chartDetails.graphCoordY} onDragStart={onMouseOut} ref={groupRef}>
          ...
          {chartDetails.dataSet.map((rect, i) => <Fragment key={i}>
            ...
            <Text key={-i - chartDetails.dataSet.length} text={chartDetails.horiArrow.months} fontSize={chartDetails.graphWidth > 300 ? 15 : 10} x={30 + chartDetails.padding + i * chartDetails.xScale} y={chartDetails.graphHeight - chartDetails.padding / 1.1} visible={chartDetails.dataSet.length < 5 || chartDetails.graphWidth > 200} />
            {console.log(chartDetails.padding)}
          </Fragment>)}
        </Group>
        {chartDetails.toolTip.active && <Tooltip y={chartDetails.toolTip.y} msg={chartDetails.toolTip.msg} />}
      </Layer>
    </Stage>
  );
}
