...
const App = ({ shapeProps, isSelected, onSelect, onChange }) => {
  ...
  const [toolTip, setToolTip] = useState(chartDetails.toolTip);
  ...
  const onMouseOver = (e) => {
    ...
    console.log(e.target.getPointerPosition());
    setToolTip((prev) => {
      return {
        active: true,
        x: shapeRef.current.getStage().getPointerPosition().x,
        y: shapeRef.current.getStage().getPointerPosition().y,
