...
const Rectangle = ({ graphWidth, graphHeight, padding, topValue,
    ...
    return (
        ...
        />, <Text key={-i - 1} text={i < inputs.length - 2 ? i * Math.ceil(Math.ceil(topValue / 10) * 10 / Math.floor(inputs.length / 10) * 10) : null} fontSize={graphHeight > 300 ? 15 : 10} x={padding} y={graphHeight - padding - i * ((graphHeight - padding) / (inputs.length - 2.9))} visible={inputs.length < 5 || graphWidth > 200} />,
