...
const Rectangle = ({ graphWidth, graphHeight, padding, topValue,
    ...
    return (
        ...
        />, <Text key={-i - 1} text={i < 10 ? i * Math.ceil(Math.floor(topValue * 1.2 / 10) * 10 / Math.floor(inputs.length)) : null} fontSize={graphHeight > 300 ? 15 : 10} x={padding} y={graphHeight - padding - i * ((graphHeight - padding) / (inputs.length - 2.9))} visible={inputs.length < 5 || graphWidth > 200} />,
