...
const Rectangle = ({ graphWidth, graphHeight, padding, topValue,
    ...
    return (
        inputs.map((item, i) => [<Rect
            ...
            y={graphHeight - padding - item * yScale * 1}
            width={xScale / 2}
            height={item * yScale * 1}
            fill={item > 20 ? "red" : "blue"}
            shadowBlur={10}
            onMouseOver={(e) => onMouseOver(e)}
            onMouseOut={(e) => onMouseOut(e)}

        />, <Text key={-i - 1} text={i < inputs.length ? i * Math.ceil(Math.ceil(topValue / 10) * 10 / Math.floor(inputs.length)) : null} fontSize={graphHeight > 300 ? 15 : 10} x={padding} y={graphHeight - 2 * padding - i * ((graphHeight - padding) / (inputs.length))} visible={inputs.length < 5 || graphWidth > 200} />,
