...
const Rectangle = ({ graphWidth, graphHeight, padding, topValue,
    ...
    return (
        ...
        />, <Text key={-i - 1} text={i < inputs.length ? i * Math.round(Math.ceil(topValue / 10) * 10 / Math.ceil(inputs.length)) : null} fontSize={graphHeight > 300 ? 15 : 10} x={padding} y={graphHeight - padding - i * ((graphHeight - padding) / (inputs.length))} visible={inputs.length < 5 || graphWidth > 200} />,
